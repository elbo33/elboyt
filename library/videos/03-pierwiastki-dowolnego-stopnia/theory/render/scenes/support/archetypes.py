...
def stage_figure(scene, figure, *, question=None, caption=None, reveal=None, pace="slow"):
    ...
    if question:
        ...
        q.to_edge(UP, buff=1.78 if not IS_VERTICAL else 2.75)
    ...
    if IS_VERTICAL:
        ...
        top = (q.get_bottom()[1] - 0.72) if q is not None else (FRAME_H / 2 - 1.32)
...
def stage_card(scene, *, centerpiece, annotations=None, strip=None, caption=None, pace="slow"):
    ...
    if IS_VERTICAL:
        ...
        _fit(card, FRAME_W - 1.8, 1.85)
        card.move_to([0, 0.72 if annotations else 0.16, 0])
    ...
    for sub, label in (annotations or []):
        ...
        lab = small_label(label, 0.28 if not IS_VERTICAL else 0.34, ACCENT).next_to(box, DOWN, buff=0.28)
    ...
    if strip:
        ...
        s.move_to([0, card.get_bottom()[1] - 1.4 if IS_VERTICAL else max(card.get_bottom()[1] - 1.05, -1.95), 0])
...
def stage_model(scene, *, cards, tagline=None, pace="slow"):
    ...
    _fit(group, FRAME_W - 1.4, FRAME_H - (3.35 if not IS_VERTICAL else 2.8), min_fill=0.62)
    if not IS_VERTICAL:
        group.shift(DOWN * 0.18)
