...
from ..config import IS_VERTICAL, hold, rt
from ..style import (
    ANSWER_Y,
    CAPTION_Y,
    FONT,
...
def worked_beat(scene, ex, beat):
    ...
    if name == "present":
        ...
        scene.play(FadeIn(stmt, shift=0.15 * UP), run_time=rt(1.1))
        hold(scene, 5.0)
        return
    ...
    if name == "restate":
        scene.play(FadeIn(strip, shift=0.1 * UP), run_time=rt(1.0))
        hold(scene, 1.0)
        scene.play(Circumscribe(strip, color=ACCENT, buff=0.15), run_time=rt(1.4))
        hold(scene, 0.6)
        scene.play(FadeIn(chip, shift=0.2 * LEFT), run_time=rt(0.8))
        hold(scene, 3.5)
        return
    ...
    if name == "plan":
        ...
        scene.play(Write(formula), run_time=rt(1.3))
        scene.play(FadeIn(_note(ex["plan_note"]), shift=0.15 * UP), run_time=rt(0.8))
        hold(scene, 3.5)
        return
    ...
    if name == "substitute":
        ...
        scene.play(TransformFromCopy(base[-1], filled), run_time=rt(1.2))
        hold(scene, 0.6)
    ...
    if name.startswith("compute-"):
        ...
        if comp["kind"] == "arith":
            ...
            scene.play(TransformFromCopy(cur, new_line), run_time=rt(1.1))
            hold(scene, 2.6)
        else:
            ...
            scene.play(FadeOut(added), FadeOut(note),
                       TransformFromCopy(cur, new_line), run_time=rt(1.0))
            hold(scene, 1.8)
        return
    ...
    if name == "result":
        ...
        scene.play(Create(box), run_time=rt(0.8))
        scene.play(Circumscribe(VGroup(stack[-1], box), color=SECONDARY), run_time=rt(1.2))
        scene.play(FadeIn(sentence, shift=0.2 * UP), run_time=rt(0.9))
        hold(scene, 4.5)
        return
    ...
    if name == "insight":
        ...
        scene.play(Write(card), run_time=rt(1.4))
        scene.play(Circumscribe(card, color=ACCENT, buff=0.3), run_time=rt(1.4))
        hold(scene, 5.5)
        return
    ...
    if name == "check":
        ...
        if chk["kind"] == "list":
            ...
            scene.play(Circumscribe(row[-1], color=SECONDARY), run_time=rt(1.0))
        else:
            line = MathTex(chk.get("line", ""), color=FOREGROUND).scale(0.7).set_y(0.3)
            scene.play(Write(line), run_time=rt(1.2))
        scene.play(FadeIn(_note(chk["note"]), shift=0.15 * UP), run_time=rt(0.8))
        hold(scene, 4.5)
        return
